...
// Ambient temperature around device. Lilygo has no temp sensor
int temperature = 22;
int vref = 1100;
...
void setup() {
  ...
  fb = (uint8_t *)heap_caps_malloc(EPD_WIDTH * EPD_HEIGHT / 2, MALLOC_CAP_SPIRAM); // orig. old way
  ...
  if (StartWiFi() == WL_CONNECTED && SetupTime() == true) {
    //if ((CurrentHour >= WakeupTime && CurrentHour <= SleepTime)) {
      ...
      if (true || (RxWeather && RxForecast)) { // Only if received both Weather or Forecast proceed
        ...
        enum EpdDrawMode mode = (enum EpdDrawMode)(MODE_GL16 | MODE_PACKING_2PPB | PREVIOUSLY_WHITE);
        epd_draw_base(epd_full_screen(), fb, epd_full_screen(), mode, temperature, NULL, WAVEFORM);
        t2 = Millis();
        epd_poweroff_all();
      }
    //}
...
void DisplayAstronomySection(int x, int y) {
  setFont(OpenSans12B);
  drawString(x - 55, y + 4, ConvertUnixTime(WxConditions[0].Sunrise).substring(0, 5) + " " + TXT_SUNRISE, LEFT);
  drawString(x - 55, y + 34, ConvertUnixTime(WxConditions[0].Sunset).substring(0, 5) + " " + TXT_SUNSET, LEFT);
  time_t now = time(NULL);
  struct tm * now_utc  = gmtime(&now);
  const int day_utc = now_utc->tm_mday;
  const int month_utc = now_utc->tm_mon + 1;
  const int year_utc = now_utc->tm_year + 1900;
  drawString(x + 50, y + 100, MoonPhase(day_utc, month_utc, year_utc, Hemisphere), LEFT);
  DrawMoon(x + 185, y - 40, day_utc, month_utc, year_utc, Hemisphere);
}
